...
// this Value type is used to hold values of args which could be passed multiple times
// e.g. $ command -x 1 -x 2 -x 3
...
func migrateSystem(migration connect.MigrationPath) (string, error) {
	...
	for _, p := range migration {
		...
		if service.ObsoletedName != "" {
			...
			connect.MigrationRemoveService(service.ObsoletedName)
		}
		...
		VerboseOut.Println(msg)
		connect.MigrationAddService(service.URL, service.Name)
...
func zypperDupArgs() []string {
	...
	// supplementary: allow-vendor-change/no-allow-vendor-change
	// no-a-v-c is passed to `zypper dup` even if neither a-v-c nor
	// no-a-v-c flag was passed to `zypper migration`
	// if both were passed, no-a-v-c has higher priority because
	// flag.Visit() orders flags alphabetically
